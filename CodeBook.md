...
\hypertarget{getting-and-cleaning-data-course-project}{%
\subsection{Getting and Cleaning Data Course
Project}\label{getting-and-cleaning-data-course-project}}
...
\hypertarget{dataset-features}{%
\subsubsection{Dataset Features}\label{dataset-features}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{listoffeatures <-}\StringTok{ }\KeywordTok{fread}\NormalTok{(}\StringTok{"./UCI HAR Dataset/features.txt"}\NormalTok{)}
\KeywordTok{select}\NormalTok{(listoffeatures,}\DecValTok{2}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{print}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\KeywordTok{nrow}\NormalTok{(listoffeatures))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{activitylables <-}\StringTok{ }\KeywordTok{fread}\NormalTok{(}\StringTok{"./UCI HAR Dataset/activity_labels.txt"}\NormalTok{)}
\CommentTok{#print(activitylables,1:n(activitylables$V2))}
\KeywordTok{select}\NormalTok{(activitylables,}\DecValTok{2}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{print}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\KeywordTok{nrow}\NormalTok{(activitylables))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{trainsetx <-}\StringTok{ }\KeywordTok{fread}\NormalTok{(}\StringTok{"./UCI HAR Dataset/train/X_train.txt"}\NormalTok{)}
\NormalTok{trainlabelsety <-}\StringTok{ }\KeywordTok{fread}\NormalTok{(}\StringTok{"./UCI HAR Dataset/train/y_train.txt"}\NormalTok{)}
\NormalTok{trainsubject <-}\StringTok{ }\KeywordTok{fread}\NormalTok{(}\StringTok{"UCI HAR Dataset/train/subject_train.txt"}\NormalTok{)}
\NormalTok{testingsetx <-}\StringTok{ }\KeywordTok{fread}\NormalTok{(}\StringTok{"UCI HAR Dataset/test/X_test.txt"}\NormalTok{)}
\NormalTok{testlabelsety <-}\KeywordTok{fread}\NormalTok{(}\StringTok{"UCI HAR Dataset/test/y_test.txt"}\NormalTok{)}
\NormalTok{testsubject <-}\StringTok{ }\KeywordTok{fread}\NormalTok{(}\StringTok{"UCI HAR Dataset/test/subject_test.txt"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\hypertarget{merges-the-training-and-the-test-sets-to-create-one-data-set.}{%
\subsubsection{1. Merges the training and the test sets to create one
data
set.}\label{merges-the-training-and-the-test-sets-to-create-one-data-set.}}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#combine sub datasets}
\NormalTok{activity <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(trainlabelsety,testlabelsety)}
\NormalTok{features <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(trainsetx,testingsetx)}
\NormalTok{subject <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(trainsubject,testsubject)}
\CommentTok{#set column names }
\KeywordTok{colnames}\NormalTok{(activity) <-}\StringTok{ "V2"}
\CommentTok{# transpose the column of feature names and set the feature column names}
\KeywordTok{colnames}\NormalTok{(features) <-}\StringTok{ }\KeywordTok{t}\NormalTok{(listoffeatures[,}\DecValTok{2}\NormalTok{])}
\CommentTok{#combine (Merge) into one dataset}
\NormalTok{combineddataset <-}\StringTok{ }\KeywordTok{cbind}\NormalTok{(subject, activity, features)}
\KeywordTok{print}\NormalTok{(}\KeywordTok{str}\NormalTok{(combineddataset))}
\end{Highlighting}
\end{Shaded}
...
\section{2. Extracts only the measurements on the mean and standard
deviation for each
measurement.}\label{extracts-only-the-measurements-on-the-mean-and-standard-deviation-for-each-measurement.}}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#get the columns that have mean and standard deviation in the label}
\NormalTok{columnsmeanstd <-}\StringTok{ }\KeywordTok{grep}\NormalTok{(}\StringTok{".*mean.*|.*std.*"}\NormalTok{, }\KeywordTok{names}\NormalTok{(combineddataset), }\DataTypeTok{ignore.case=}\OtherTok{TRUE}\NormalTok{)}
\CommentTok{# format list of columns with activity, subject, and the features that have means and std deviation}
\NormalTok{columnsneededlist <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{,columnsmeanstd)}
\CommentTok{# create the formated data frame}
\NormalTok{columnsneeddf <-}\StringTok{ }\NormalTok{combineddataset[, columnsneededlist, with=}\OtherTok{FALSE}\NormalTok{]}
\KeywordTok{print}\NormalTok{(}\KeywordTok{str}\NormalTok{(columnsneeddf))}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{use-descriptive-activity-names-to-name-the-activities-in-the-data-set}{%
\section{3. Use descriptive activity names to name the activities in the
data
set}\label{use-descriptive-activity-names-to-name-the-activities-in-the-data-set}}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#check activity number in column and replace with corresponding activity label}
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\DecValTok{6}\NormalTok{)\{ columnsneeddf}\OperatorTok{$}\NormalTok{V2[columnsneeddf}\OperatorTok{$}\NormalTok{V2 }\OperatorTok{==}\StringTok{ }\NormalTok{i] <-}\StringTok{ }\KeywordTok{c}\NormalTok{(activitylables}\OperatorTok{$}\NormalTok{V2[i])\}}
\KeywordTok{print}\NormalTok{(}\KeywordTok{str}\NormalTok{(columnsneeddf}\OperatorTok{$}\NormalTok{V2))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(}\KeywordTok{unique}\NormalTok{(columnsneeddf}\OperatorTok{$}\NormalTok{V2))}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{appropiately-labels-the-data-set-with-descriptive-variable-names}{%
\section{4. Appropiately labels the data set with descriptive variable
names}\label{appropiately-labels-the-data-set-with-descriptive-variable-names}}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#feature column names were already set previously}
\CommentTok{#finish setting column names Subject and Activity}
\KeywordTok{colnames}\NormalTok{(columnsneeddf)[}\KeywordTok{colnames}\NormalTok{(columnsneeddf) }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"V1"}\NormalTok{, }\StringTok{"V2"}\NormalTok{)] <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Subject"}\NormalTok{, }\StringTok{"Activity"}\NormalTok{)}
\KeywordTok{print}\NormalTok{(}\KeywordTok{names}\NormalTok{(columnsneeddf))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{avgtidydata <-}\StringTok{ }\NormalTok{columnsneeddf }\OperatorTok{%>%}
\StringTok{                             }\KeywordTok{group_by}\NormalTok{(Subject, Activity) }\OperatorTok{%>%}
\StringTok{                             }\KeywordTok{summarize_each}\NormalTok{(}\KeywordTok{funs}\NormalTok{(mean))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(avgtidydata)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Write the tidy dataset to a file}
\KeywordTok{fwrite}\NormalTok{(avgtidydata, }\DataTypeTok{file=}\StringTok{"./avgtidydata.txt"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
